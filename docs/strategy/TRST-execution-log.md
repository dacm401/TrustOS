...
## 2026-08-29 — 数据主权原则确立（ADR-002）+ 主权数据演进路线规划

> Boss 拍板并指出此前认知偏差。本段记录决策依据与认知修正过程。

### 背景：Boss 的战略质疑

Boss 指出：现在大模型 App 的历史对话全部存云端、可查询——**云端不是已经
把自主权完全交出去了吗**？我们用 Manager+Worker，不就是为了把 Manager 的
原始 prompt 留着、只上传加工过的，从而保留自主权吗？
...
此前顾虑「本地存原文会被攻击」，却对已发生且完全失控的云端泄露习以为常——
**对可控的小风险过度警惕，对失控的大风险无感**。

**更深的洞察**：旧护栏「raw 不落库」实际上**削弱**了 Manager+Worker 架构。
若本地不留原文，则：本地什么都没有（加工完即弃），云端有你外发的每一次内容
⇒ **唯一拥有完整数据的一方，是你无法控制的一方** ⇒ 自主权是空话。

**核心命题**：加工解决「**少泄露**」，留存解决「**谁拥有**」。
两者缺一不可。

**定位修正**：差异不在"更安全"，而在**主权归属**——
同等便利（历史可查、上下文连续），但数据在你手上。

### 决策（ADR-002，ACCEPTED）

原始 prompt 与对话数据**必须在本机留存（加密）**，作为数据主权的物理基础。

四条配套原则：
1. 加工解决少泄露，留存解决谁拥有（缺一不可）
2. **能带走的数据才是你的数据**（须支持打包/加密/导出/迁移）
3. 主权数据是**演进资产**（为本地模型读取预留格式与接口）
4. 按敏感度分级，而非一刀切
...
| 层 | 内容 | 位置 | 外发 |
|---|---|---|---|
| L1 原始意图 | 完整 prompt、上下文、原文 | **本地（加密）** | ❌ 从不 |
| L2 加工产物 | 脱敏/摘要后的 brief | 外发 → 云端留副本 | ✅ 唯一外发层 |
| L3 模型输出 | 返回结果 | 回落本地 | — |
| L4 云端副本 | L2 的副本 | 云端（不可控） | — |

云端永远只有 L2；L1 永远在你手上。L4 是不可消除的已知损失，
但因只含 L2，损失被限定在加工后的信息量。

### 演进路线（RFC-001 v2，新增）

| Phase | 目标 | 状态 |
|---|---|---|
| 1 | 主权数据层——数据开始在本机积累 | **待拍板** |
| 2 | 主权数据包——可打包、可加密、可迁移 | 方向已定 |
| 3 | 本地模型读取——**先 RAG 后微调** | 方向已定 |
...
**新识别的关键难题**：**加密与可检索性的矛盾**
`at-rest 加密` ≠ `密文可检索`。同态加密性能差 3~5 个数量级，不现实。
方案：**索引/元数据明文 + 内容加密**（分层，与外发加工同一哲学）。
此矛盾**无法消除，只能管理**。

### 诚实接受的局限

- 本地泄露的**浓缩性**风险：本地存的是高度浓缩的完整画像，价值密度高于单次对话
- **"机器被控"应用层无解**：at-rest 加密防设备丢失，防不了机器被控后密钥同失
- 加密与检索的矛盾（见上）
- **本地模型能力有限**：不能替代云端复杂推理；定位为"个性化/记忆检索"

### 文档产出

| 文档 | 说明 |
|---|---|
| `ADR-002-data-sovereignty-principle.md` | **新建** — 数据主权原则（决策依据 + 分层模型 + 局限） |
| `RFC-001-local-memory-distillation.md` | **v2 重构** — 动机重排（主权为首要、粘性为次要）、新增 §2 分层主权模型、§3 演进路线、§8.1 加密与检索矛盾专项 |
| `CURRENT-STATUS.md` | 更新（新增 §3.4 数据主权原则，待办改为 Phase 1 拍板） |
| `TRST-doc-map.md` | 更新决策记录索引 |

### 待办

- **待 Boss 拍板**：RFC-001 Phase 1 的 5 项（加密方案 / L2 是否做 /
  保留期 / 低置信度处理 / 是否回溯历史）。Phase 2-3 仅需确认方向
- 同步 `TRST-0` 旧护栏表述（已被 ADR-001/002 变更，原文未同步）

*Last updated: 2026-08-29*

---

## 2026-08-29 — 冷热分离策略确定（RFC-001 决策拍板）

> Boss 拍板 RFC-001 Phase 1 的加密与保留策略。
...
| 3 | 原文保留期 | **长期保留，不设保留期**（原建议 90 天作废） |
| 6 | 归档触发时机 | 手动 + 按时间（每月）+ 按大小（阈值），任一满足即提示 |

### 冷热分离模型

| 层 | 加密 | 可检索 | 用途 | 保留 |
|---|---|---|---|---|
| **热层**（近期原文 + Memory） | ❌ 明文 | ✅ 直接检索、本地模型可读 | 日常查询 | 直到归档 |
| **冷层**（归档包） | ✅ 强加密（口令派生） | ❌ 需解密导入 | 长期保存/备份/迁移 | **永久** |

**关键设计：小数据常驻，大数据归档**
- Memory（蒸馏物）体积小、价值密度高 → **永不归档、永不清理**，
  始终可检索（这是「越来越懂你」的核心）
- 原文体积大、访问频率低 → 周期打包加密归档后长期保留

### 为什么这个方案重要

它**化解了此前认定的无解难题**：「加密与可检索性的矛盾」（原 RFC §8.1）。

原判断：at-rest 加密 ≠ 密文可检索，同态加密性能差 3~5 个数量级不现实，
故矛盾「无法消除，只能管理」。

化解思路：这两类数据的**使用频率根本不同**——
- 日常要检索、要给本地模型读的是**近期数据** → 明文，牺牲静态保护换可用性
- 归档的是**低频历史** → 加密，牺牲即时可用性换安全

于是「可检索」与「受保护」不再需要在同一份数据上同时满足。
这是**用数据分层替代加密技术难题**的典型解法。

### 因决策而更新的文档

| 文档 | 更新 |
|---|---|
| `RFC-001` | §0 一句话概括、§2 分层模型、§4 策略一（冷热 + 长期保留）、§4.1 加密策略重写、§6 坏处表（8 项，新增口令丢失风险）、§7 Phase0/2、§8 风险表、§8.1 矛盾已化解、§10.1 三项已拍板 |
| `ADR-002` | 新增原则 5（冷热分离）、更新局限（矛盾已化解 + 归档口令丢失） |

### 新增认知的风险

| 风险 | 说明 |
|---|---|
| 热层明文 | 机器被控时近期数据可读（应用层无解），建议 OS 全盘加密 |
| 归档口令丢失 | 口令派生密钥 ⇒ 口令丢失 = 归档不可恢复，须 UX 强提示 |
| 归档膨胀 | 长期保留是有意为之（主权资产），**不做自动删除**，需容量提示 |

### 待定（不阻塞启动）

2 是否做 L2 语义蒸馏 / 4 低置信度记忆处理 / 5 是否回溯历史对话
—— 建议采纳「我的建议」，Boss 无异议则视为同意。
