...
const DEFAULT_CORE_RULES = [
  ...
  // ── 任务边界隔离（与 Worker 侧的边界指令对称）────────────────────────
  // Worker 侧早就有「只使用 Task Brief 提供的信息，不要读取任何外部历史对话」，
  // Manager 侧一直缺这条，导致模型把历史里的问题并入当前任务。
  // 真实事故：历史里有「用Python写快速排序」，用户接着问「天为啥蓝」，
  // Manager 生成了合并任务「用Python编写快速排序算法，并解释天空为什么是蓝色的」，
  // Worker 据此输出快速排序代码 —— 用户拿到的是上一条问题的答案。
  "【任务边界】只针对 [current_user_input] 中「用户本轮输入」做决策和拆解",
  "【任务边界】对话历史仅用于理解指代关系（如『它』『再改一下』『刚才那个』），"
    + "其中的问题都已处理完毕，严禁把历史里出现过的任何问题并入当前任务",
  "【任务边界】即使历史上的任务看起来还没给出最终结果，也不要替用户续做；"
    + "用户没再次提起，就当他已经不关心了",
];
