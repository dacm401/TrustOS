...
>
> ✅ **2026-08-30 已修复**（Boss 决策「恢复读取能力」）：
> - `DelegationArchiveRepo.findSimilar()` —— 恢复「查档案库」检索
> - `src/services/archive-replay.ts` —— 档案命中快路径，**零模型调用**直接返回
> - 多重安全门控：相似度阈值（默认 0.9）/ 时效问题双向排除 /
>   仅 replay 有结果的 completed 条目 / 可一键关闭
> - **关键修复**：相似度用**非对称覆盖率**而非 Jaccard。Jaccard 受长度差稀释
>   （「写一个快速排序」vs「写一个快速排序算法」仅 0.75），配 0.9 阈值会导致
>   replay 只能命中完全相同的问句、形同虚设。覆盖率同时是**安全方向**：
>   当前问题若问了档案里没有的内容，覆盖率下降 → 正确拒绝 replay。
> - 端到端实测：`[archive-replay] hit score=1.00 — 0 model calls` ✅
>
> ⚠️ 实测发现：**当前路由下问题多走 `direct_answer`，委托档案几乎不再产生**
> （验证时表中 0 条记录，需手工插入测试条目才测得命中）。故 replay 实际收益
> 取决于委托发生频率，应用 `archive_replays_total` 指标持续观察。
