...
| G4 | Delegation Learning Loop | ⏳ TODO（stats fields done） | Sprint 51→ |
...
| Phase D | 🟡 G1/G2/G3 DONE，G4 TODO（stats fields done） | delegation_logs 统计字段已补充（selected_role/exec_input_tokens/cost_saved_vs_slow），学习闭环待完成 |
| API 层 | ✅ ~20个端点全部实现 | — |
...
**P0 缺口（阻塞学习闭环）**：
1. **G4 Delegation Learning Loop** — `delegation_logs` 事实表统计字段已补充（selected_role/exec_input_tokens/cost_saved_vs_slow，commit 8f4cbeb），日志 pipeline 待完成
