...
/** 需要委托慢模型的任务类型 */
const NEED_DELEGATION_INTENTS = new Set(["reasoning", "math", "code", "research"]);

/** 低复杂度阈值（低于此值不委托） */
const LOW_COMPLEXITY_THRESHOLD = 25;

/** 高复杂度关键词（有这些词的意图直接委托） */
const HIGH_COMPLEXITY_KEYWORDS = [
  /分析|研究|调研|对比|比较|评估/i,
  /写.*代码|实现.*算法|debug|调试/i,
  /翻译成|译成|翻译为/i,
  /总结.*全文|概括.*内容|提炼.*要点/i,
];
...
【你的职责】
- 快速回复用户，不需要深度思考
- 如果你需要委托慢模型，先给用户一个简洁的初步回复，再触发后台慢模型
- 你的回复要自然、口语化，不要列清单，不要废话

【委托场景】
如果用户的问题是复杂分析、代码生成、数学推导、研究报告等，需要慢模型处理时：
1. 先给用户一个简洁的初步回复（1-3句话）
2. 在回复末尾告知用户"我让慢模型在后台处理，稍后给你完整答案"
3. 不要等慢模型，直接返回

【直接回复场景】
闲聊、简单问答、情绪回应：直接给答案，1-2句话足够。`;
  }
  return `You are SmartRouter Pro's fast model assistant.

【Your role】
- Reply quickly, no deep thinking needed
- If you need to delegate to the slow model, give a brief initial reply first, then trigger background processing
- Keep responses natural and conversational

【Delegation scenarios】
For complex analysis, code generation, math, research: give a brief reply first, then tell the user the slow model is processing in background.

【Direct reply scenarios】
Casual chat, simple Q&A, emotional responses: answer directly in 1-2 sentences.`;
}
...
async function triggerSlowModelBackground(input: SlowModelBackgroundInput): Promise<void> {
  const { taskId, message, user_id, session_id, fast_reply, reqApiKey } = input;
