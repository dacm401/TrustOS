...
The Manager operates within the **Manager Loop** — a fast, user-facing control loop that does not perform heavy execution. The Worker operates within the **Worker Loop** — a long-running, asynchronous execution loop. Tool and resource access flows through the **Action Loop** — a deterministic, policy-driven guard.

```text
Manager Loop:  user control, delegation, approval, summarization
Worker Loop:   task execution, progress reporting
Action Loop:   policy enforcement, tool access, audit
```

These three loops must not be merged into one monolithic agent loop. See `docs/architecture/Loop-Separation-RFC.md` for the full design.
...
Worker receives decision
  ↓
User sees relevant events through Manager Workspace
```
