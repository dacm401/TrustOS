...
// ── ManagerDecision (Phase 2 — 替代旧的意图/复杂度打分器) ─────────────────

/**
 * Fast 模型输出的四种标准动作之一。
 * 替代旧的【SLOW_MODEL_REQUEST】正则匹配方案。
 */
export type ManagerAction =
  | "direct_answer"      // 直接回复
  | "ask_clarification" // 请求澄清
  | "delegate_to_slow"  // 委托慢模型
  | "execute_task";      // 触发执行模式（execute mode）

/**
 * ManagerDecision — Fast 模型的结构化决策输出。
 * 通过 parseManagerDecision() 从文本中解析并校验。
 */
export interface ManagerDecision {
  version: "v1";
  action: ManagerAction;
  confidence: number;     // 0.0–1.0
  reasoning: string;      // 简短推理（1-2句话）
  // action=ask_clarification 时
  clarification?: {
    question_text: string;
    options?: string[];  // 多选时提供
  };
  // action=delegate_to_slow 时
  delegation?: {
    action: "research" | "analysis" | "code" | "creative" | "comparison";
    task: string;         // 核心任务描述 <100字
    constraints: string[];
    query_keys: string[];
