/**
 * Vitest config for S80P Resume Decision Persistence V0 tests.
 *
 * 运行方式:
 *   npx vitest run --config vitest.s80p.config.ts
 *
 * 测试范围:
 *   tests/services/human-review/human-review-decision-persist.test.ts       (unit)
 *   tests/services/human-review/human-review-decision-persist-boundary.test.ts
 *   tests/services/human-review/human-review-decision-persist-e2e.test.ts    (real DB)
