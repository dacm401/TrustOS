...
## 7.1 Loop Separation Principle

TrustOS does not put Manager, Worker, and Tool execution into one monolithic agent loop.

TrustOS separates:

- **Manager Loop**: user control and trust mediation — fast, user-facing, does not perform heavy execution
- **Worker Loop**: long-running delegated execution — may be slow, async, pausable, retryable
- **Action Loop**: deterministic resource access and audit — policy-driven, avoids LLM, does not chat with users
...
A single chat window forces Manager control, Worker execution, Action decisions, approvals, and multiple delegated tasks into one mixed stream. Loop Separation ensures each layer has its own runtime characteristics, latency budget, and user-facing representation.

See `docs/architecture/Loop-Separation-RFC.md` for the full design.
...
- a full Agent Engine PaaS in the current roadmap,
- a remote sandbox platform as the immediate product goal.
...
- S100P: Manager Workspace v1 — Loop Separation in UX, Make the Manager Visible
