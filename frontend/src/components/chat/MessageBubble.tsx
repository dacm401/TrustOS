          ...
          <>
            ...
            {(decision?.execution || routingLayer) && (
              ...
              >
                ...
                {/* MWT-1: Gateway 观察状态 badge */}
                {observed !== undefined && (
                  <span
                    className="text-[9px] px-1.5 py-0.5 rounded font-medium"
                    style={{
                      backgroundColor: observed
                        ? "rgba(16,185,129,0.12)"
                        : "rgba(245,158,11,0.12)",
                      color: observed ? "var(--accent-green)" : "var(--accent-amber)",
                    }}
                    title={
                      observed
                        ? "Trust Layer 在线 - 此消息已被 Gateway 观察"
                        : "Trust Layer 离线 - 此消息未被 Gateway 观察"
                    }
                  >
                    {observed ? "Observed ✓" : "Unobserved ⚠️"}
                  </span>
                )}
                {decision?.execution && (
                  <>
