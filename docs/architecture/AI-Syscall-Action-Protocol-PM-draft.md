...
### 1.1 Loop Separation Context

**Action Protocol belongs to Action Loop, not Manager Loop.** Per `docs/architecture/Loop-Separation-RFC.md`:
...
Trust Kernel
  ↓ Audit Event
Manager (ManagerLoop)
  ↓ User-visible explanation when relevant
User
```
...
| Visibility | Where It Appears | Loop |
|---|---|---|
| `silent_audit` | Audit log only | ActionLoop → AuditEvent |
| `session_timeline` | Session Detail panel | WorkerLoop → SessionEvent |
| `approval_required` | Approval card in Session Detail | ActionLoop → Approval → ManagerLoop |
| `manager_chat_summary` | Manager Conversation (summary line) | WorkerLoop → ManagerLoop |
| `trust_report_only` | Trust Report only | Aggregated at session end |
| `critical_alert` | Top bar alert + Manager chat | Any loop → ManagerLoop |
...
- Worker completed milestone → `manager_chat_summary`
